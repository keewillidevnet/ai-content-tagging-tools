...
async function tagAudioFile() {
  console.log('🎵 Tagging audio file with AI metadata...');
  ...
  try {
    ...
    console.log('✅ Audio file tagged successfully!');
...
async function tagImageFile() {
  console.log('🖼️ Tagging image file with AI metadata...');
  ...
  try {
    ...
    console.log('✅ Image file tagged successfully!');
...
async function batchProcessMultimedia() {
  console.log('📦 Batch processing multimedia files...');
...
async function validateMultimediaMetadata() {
  console.log('🔍 Validating multimedia metadata integrity...');
    ...
    try {
      ...
      if (hasMetadata) {
        ...
        if (metadata) {
          ...
          console.log(`- Checksum verified: ${metadata._verified ? '✅' : '❌'}`);
          console.log(`- Signature valid: ${metadata._signatureValid ? '✅' : '❌'}`);
      ...
      console.error(`❌ Error validating ${testFile.path}:`, error.message);
    }
  }
}
...
async function removeMultimediaMetadata() {
  console.log('🗑️ Removing AI metadata from multimedia files...');
  ...
  for (const testFile of testFiles) {
    try {
      ...
      console.log(`✅ Cleaned file saved: ${testFile.output}`);
      console.log(`AI metadata removed: ${!hasMetadata ? '✅' : '❌'}`);
      
    } catch (error) {
      console.error(`❌ Error cleaning ${testFile.path}:`, error.message);
    }
  }
}
...
async function performanceTest() {
  ...
  try {
    ...
    console.log(`✅ Processed 10 x 1MB audio files in ${audioTime}ms`);
    console.log(`⚡ Average: ${audioTime / 10}ms per file`);
    
  } catch (error) {
    console.error('❌ Audio performance test failed:', error.message);
  }
  ...
  try {
    ...
    console.log(`✅ Processed 10 x 1MB image files in ${imageTime}ms`);
    console.log(`⚡ Average: ${imageTime / 10}ms per file`);
    
  } catch (error) {
    console.error('❌ Image performance test failed:', error.message);
  }
}
...
async function compareMetadataFormats() {
  console.log('🔄 Comparing metadata formats...');
  ...
  try {
    ...
    console.log(`XMP - Embedded: ${xmpTagged.length} bytes, Extracted: ${xmpExtracted ? '✅' : '❌'}`);
    ...
    console.log(`EXIF - Embedded: ${exifTagged.length} bytes, Extracted: ${exifExtracted ? '✅' : '❌'}`);
    
  } catch (error) {
    console.error('❌ Format comparison failed:', error.message);
  }
}
...
async function testErrorHandling() {
  ...
  try {
    ...
    console.log('❌ Should have thrown error for invalid metadata');
  } catch (error) {
    console.log('✅ Correctly caught invalid metadata error');
  }
  ...
  try {
    ...
    console.log('❌ Should have handled corrupted file');
  } catch (error) {
    console.log('✅ Correctly handled corrupted file');
  }
  ...
  try {
    ...
    console.log(`✅ Empty file check: ${result ? 'has metadata' : 'no metadata'}`);
  } catch (error) {
...
async function runDemo() {
  console.log('🚀 AI Content Tagging Tools - Multimedia Support Demo\n');
  console.log('=' * 60);
  ...
  try {
    ...
    console.log('🎉 All multimedia demos completed successfully!');
    
    console.log('\n📚 Integration complete! Your repository now supports:');
    console.log('- ✅ Audio ID3 Tags (MP3, M4A, FLAC, WAV)');
    console.log('- ✅ Image EXIF/XMP (JPEG, with TIFF/PNG planned)');
    console.log('- ✅ RFC-compliant metadata embedding');
...
async function testAudioFormats() {
  console.log('🎵 Testing different audio format support...');
  ...
  for (const format of formats) {
    try {
      ...
      console.log(`✅ ${format}: ${result.length} bytes processed`);
    } catch (error) {
...
async function testImageFormats() {
  console.log('🖼️ Testing different image format detection...');
